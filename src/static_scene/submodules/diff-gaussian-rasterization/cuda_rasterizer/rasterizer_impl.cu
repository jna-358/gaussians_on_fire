...
int CudaRasterizer::Rasterizer::forward(
    ...
    float tan_fovy, const bool prefiltered, float *out_color, float *out_alpha,
    float *depth, bool antialiasing, int *radii, bool debug) {
  ...
  CHECK_CUDA(FORWARD::preprocess(
                 ...
                 tile_grid, geomState.tiles_touched, prefiltered, antialiasing),
             debug)
  ...
  CHECK_CUDA(FORWARD::render(tile_grid, block, imgState.ranges,
                             ...
                             out_alpha, geomState.depths, depth),
             debug)
...
void CudaRasterizer::Rasterizer::backward(
    ...
    char *img_buffer, const float *dL_dpix, const float *dL_invdepths,
    const float *dL_dout_alpha, float *dL_dmean2D, float *dL_dconic,
    float *dL_dopacity, float *dL_dcolor, float *dL_dinvdepth,
    float *dL_dmean3D, float *dL_dcov3D, float *dL_dsh, float *dL_dscale,
    float *dL_drot, bool antialiasing, bool debug) {
  ...
  CHECK_CUDA(BACKWARD::render(
                 ...
                 geomState.conic_opacity, color_ptr, geomState.depths,
                 imgState.accum_alpha, imgState.n_contrib, dL_dpix,
                 dL_invdepths, dL_dout_alpha, (float3 *)dL_dmean2D,
                 (float4 *)dL_dconic, dL_dopacity, dL_dcolor, dL_dinvdepth),
             debug);
  ...
  CHECK_CUDA(BACKWARD::preprocess(
                 ...
                 (glm::vec3 *)dL_dscale, (glm::vec4 *)dL_drot, antialiasing),
             debug);
}
