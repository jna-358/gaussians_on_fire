...
public:
  ...
  static int forward(std::function<char *(size_t)> geometryBuffer,
                     ...
                     const bool prefiltered, float *out_color, float *out_alpha,
                     float *depth, bool antialiasing, int *radii = nullptr,
                     bool debug = false);
  ...
  backward(const int P, int D, int M, int R, const float *background,
           ...
           const float *rotations, const float *cov3D_precomp,
           const float *viewmatrix, const float *projmatrix,
           const float *campos, const float tan_fovx, float tan_fovy,
           const int *radii, char *geom_buffer, char *binning_buffer,
           char *image_buffer, const float *dL_dpix, const float *dL_invdepths,
