...
__global__ void computeCov2DCUDA(int P, const float3 *means, const int *radii,
                                 ...
                                 const float *dL_dconics, float *dL_dopacity,
                                 const float *dL_dinvdepth, float3 *dL_dmeans,
  ...
  if (antialiasing) {
    ...
    const float h_convolution_scaling =
        sqrt(max(0.000025f,
  ...
  if (antialiasing) {
    ...
    const float w = h_var;
    const float denom_f =
        d_inside_root / sq(w * w + w * (x + y) + x * y - z * z);
  ...
  if (denom2inv != 0) {
    ...
    dL_dc_xx += denom2inv *
                (-c_yy * c_yy * dL_dconic.x + 2 * c_xy * c_yy * dL_dconic.y +
                 (denom - c_xx * c_yy) * dL_dconic.z);
    dL_dc_yy += denom2inv *
                (-c_xx * c_xx * dL_dconic.z + 2 * c_xx * c_xy * dL_dconic.y +
                 (denom - c_xx * c_yy) * dL_dconic.x);
    dL_dc_xy +=
        denom2inv * 2 *
        (c_xy * c_yy * dL_dconic.x - (denom + 2 * c_xy * c_xy) * dL_dconic.y +
    ...
    dL_dcov[6 * idx + 0] =
        (T[0][0] * T[0][0] * dL_dc_xx + T[0][0] * T[1][0] * dL_dc_xy +
         T[1][0] * T[1][0] * dL_dc_yy);
    dL_dcov[6 * idx + 3] =
        (T[0][1] * T[0][1] * dL_dc_xx + T[0][1] * T[1][1] * dL_dc_xy +
         T[1][1] * T[1][1] * dL_dc_yy);
    dL_dcov[6 * idx + 5] =
        (T[0][2] * T[0][2] * dL_dc_xx + T[0][2] * T[1][2] * dL_dc_xy +
         T[1][2] * T[1][2] * dL_dc_yy);

    // Gradients of loss L w.r.t. each 3D covariance matrix (Vrk) entry,
    // given gradients w.r.t. 2D covariance matrix (off-diagonal).
    // Off-diagonal elements appear twice --> double the gradient.
    // cov2D = transpose(T) * transpose(Vrk) * T;
    dL_dcov[6 * idx + 1] = 2 * T[0][0] * T[0][1] * dL_dc_xx +
                           (T[0][0] * T[1][1] + T[0][1] * T[1][0]) * dL_dc_xy +
                           2 * T[1][0] * T[1][1] * dL_dc_yy;
    dL_dcov[6 * idx + 2] = 2 * T[0][0] * T[0][2] * dL_dc_xx +
                           (T[0][0] * T[1][2] + T[0][2] * T[1][0]) * dL_dc_xy +
                           2 * T[1][0] * T[1][2] * dL_dc_yy;
    dL_dcov[6 * idx + 4] = 2 * T[0][2] * T[0][1] * dL_dc_xx +
                           (T[0][1] * T[1][2] + T[0][2] * T[1][1]) * dL_dc_xy +
                           2 * T[1][1] * T[1][2] * dL_dc_yy;
  } else {
    for (int i = 0; i < 6; i++)
      dL_dcov[6 * idx + i] = 0;
  }
...
preprocessCUDA(int P, int D, int M, const float3 *means, const int *radii,
               ...
               glm::vec4 *dL_drot, float *dL_dopacity) {
  auto idx = cg::this_grid().thread_rank();
  if (idx >= P || !(radii[idx] > 0))
    return;
...
__global__ void __launch_bounds__(BLOCK_X *BLOCK_Y) renderCUDA(
    ...
    const float *__restrict__ dL_dout_alpha, float3 *__restrict__ dL_dmean2D,
    float4 *__restrict__ dL_dconic2D, float *__restrict__ dL_dopacity,
    float *__restrict__ dL_dcolors, float *__restrict__ dL_dinvdepths) {
  ...
  // Read alpha output gradient for this pixel
  float dL_dout_alpha_val = 0;
  if (inside && dL_dout_alpha)
    dL_dout_alpha_val = dL_dout_alpha[pix_id];
  ...
  for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE) {
    ...
    for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++) {
      ...
      // Account for alpha output gradient contribution:
      // out_alpha = 1 - T_final, so d(out_alpha)/d(alpha_i) = T_final / (1 -
      // alpha_i)
      if (dL_dout_alpha)
        dL_dalpha += (T_final / (1.f - alpha)) * dL_dout_alpha_val;
...
void BACKWARD::preprocess(
    ...
    const float scale_modifier, const float *cov3Ds, const float *viewmatrix,
    const float *projmatrix, const float focal_x, float focal_y,
    const float tan_fovx, float tan_fovy, const glm::vec3 *campos,
    const float3 *dL_dmean2D, const float *dL_dconic, const float *dL_dinvdepth,
    float *dL_dopacity, glm::vec3 *dL_dmean3D, float *dL_dcolor,
  ...
  computeCov2DCUDA<<<(P + 255) / 256, 256>>>(
      ...
      viewmatrix, opacities, dL_dconic, dL_dopacity, dL_dinvdepth,
...
void BACKWARD::render(const dim3 grid, const dim3 block, const uint2 *ranges,
                      ...
                      const float *dL_invdepths, const float *dL_dout_alpha,
  ...
  renderCUDA<NUM_CHANNELS><<<grid, block>>>(
      ...
      depths, final_Ts, n_contrib, dL_dpixels, dL_invdepths, dL_dout_alpha,
