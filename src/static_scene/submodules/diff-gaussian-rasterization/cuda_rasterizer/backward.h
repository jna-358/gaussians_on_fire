...
void render(const dim3 grid, dim3 block, const uint2 *ranges,
            ...
            const float *colors, const float *depths, const float *final_Ts,
            ...
            float *dL_dcolors, float *dL_dinvdepths);
...
void preprocess(int P, int D, int M, const float3 *means, const int *radii,
                ...
                const float *view, const float *proj, const float focal_x,
                float focal_y, const float tan_fovx, float tan_fovy,
                const glm::vec3 *campos, const float3 *dL_dmean2D,
                const float *dL_dconics, const float *dL_dinvdepth,
                float *dL_dopacity, glm::vec3 *dL_dmeans, float *dL_dcolor,
                float *dL_dcov3D, float *dL_dsh, glm::vec3 *dL_dscale,
                glm::vec4 *dL_drot, bool antialiasing);
} // namespace BACKWARD
