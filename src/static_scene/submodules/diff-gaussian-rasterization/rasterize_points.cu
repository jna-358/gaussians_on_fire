...
RasterizeGaussiansCUDA(
    ...
    const bool antialiasing, const bool debug) {
  ...
  if (P != 0) {
    ...
    rendered = CudaRasterizer::Rasterizer::forward(
        ...
        out_alpha.contiguous().data<float>(), out_invdepthptr, antialiasing,
        radii.contiguous().data<int>(), debug);
  }
  return std::make_tuple(rendered, out_color, radii, geomBuffer, binningBuffer,
                         imgBuffer, out_invdepth, out_alpha);
}
...
RasterizeGaussiansBackwardCUDA(
    ...
    const torch::Tensor &dL_dout_color, const torch::Tensor &dL_dout_invdepth,
    const torch::Tensor &dL_dout_alpha, const torch::Tensor &sh,
  ...
  if (P != 0) {
    CudaRasterizer::Rasterizer::backward(
        ...
        colors.contiguous().data<float>(), opacities.contiguous().data<float>(),
        ...
        dL_dout_color.contiguous().data<float>(), dL_dout_invdepthptr,
        dL_dout_alpha.size(0) != 0 ? dL_dout_alpha.contiguous().data<float>()
                                   : nullptr,
        dL_dmeans2D.contiguous().data<float>(),
        dL_dconic.contiguous().data<float>(),
        dL_dopacity.contiguous().data<float>(),
        dL_dcolors.contiguous().data<float>(), dL_dinvdepthsptr,
        dL_dmeans3D.contiguous().data<float>(),
        dL_dcov3D.contiguous().data<float>(), dL_dsh.contiguous().data<float>(),
        dL_dscales.contiguous().data<float>(),
        dL_drotations.contiguous().data<float>(), antialiasing, debug);
  }
